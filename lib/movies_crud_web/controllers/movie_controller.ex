defmodule MoviesCrudWeb.MovieController do
  ...
  def index(conn, _params) do
    movies = MovieCrud.list_movies()
    render(conn, "index.html", movies: movies)
  end

  def new(conn, _params) do
    changeset = MovieCrud.change_movie(%Movie{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"movie" => movie_params}) do
    case MovieCrud.create_movie(movie_params) do
      {:ok, movie} ->
        conn
  ...
  def update(conn, %{"id" => id, "movie" => movie_params}) do
    ...
    case MovieCrud.update_movie(movie, movie_params) do
      {:ok, movie} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    movie = MovieCrud.get_movie!(id)
    {:ok, _movie} = MovieCrud.delete_movie(movie)
